...
namespace Resources {
  ...
  std::shared_ptr<sf::Font> load(const std::string &name) {
    ...
    if (!f->loadFromFile("resources/fonts/" + name)) {
  ...
  std::shared_ptr<sf::Texture> load(const std::string& name) {
    ...
    if (!tex->loadFromFile("resources/textures/" + name)) {
      throw("not found: " + name);
    };
    return tex;
  };
  ...
  std::shared_ptr<sf::SoundBuffer> load(const std::string& name) {
    ...
    if (!buf->loadFromFile("resources/sound/" + name)) {
      throw("not found: " + name);
    };
    return buf;
  };
  ...
  std::shared_ptr<sf::Music> load(const std::string& name) {
    ...
    if (!music->openFromFile("resources/sound/" + name)) {
      throw("not found: " + name);
    };
    return music;
  };
